...
private:
    ...
    void update(ChainType& chain, const CoefficientType& coefficients)
    {
        ...
        // Helper function for assigning the right coefficients (db/oct slopes) for the filters.
    ...
    void updateCutFilter(ChainType& chain, const CoefficientType& cutCoefficients, const Slope& lowCutSlope)
    {
        ...
        switch(lowCutSlope)
        {
            case Slope_48:
            {
                update<3>(chain, cutCoefficients); // <> is for the template parameters, and () is for the actual functions stuff.
            }
            case Slope_36:
            {
